...
const entries: Entry[] = [
  {
    name: "string: email+startsWith",
    schema: v.pipe(v.string(), v.email(), v.startsWith("a")),
  },
  {
    name: "number: integer [0,10]",
    schema: v.pipe(v.number(), v.integer(), v.minValue(0), v.maxValue(10)),
  },
  {
    name: "array<number>(length=2)",
    schema: v.pipe(v.array(v.number()), v.length(2)),
  },
  {
    name: "object strict",
    schema: v.strictObject({ a: v.string(), b: v.optional(v.number()) }),
  },
  ...
  {
    name: "set<string> minSize=1",
    schema: v.pipe(v.set(v.string()), v.minSize(1)),
  },
